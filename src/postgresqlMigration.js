...
function generatePostgresSchema(tableName, fieldAnalysis, nestedFields, arrayFields) {
  ...
  Object.values(fieldAnalysis).forEach(field => {
    ...
    if (types.includes('objectId') || field.isPrimaryKey) {
      ...
      pgType = 'TEXT'; // Changed from JSONB to TEXT
    } else {
      pgType = 'TEXT';
    }
  ...
  // Add columns for nested objects and arrays as TEXT
...
function generateMigrationStrategy(collectionName, tableName, fieldAnalysis, nestedFields, arrayFields) {
  ...
  if (complexFields.length > 0) {
    ...
    complexFields.forEach(field => {
      strategy += `   - ${field}: Store as TEXT (JSON string)\n`;
    });
    strategy += '\n';
  }
  ...
  strategy += '   - For complex fields stored as TEXT, consider using full-text search capabilities if needed\n\n';
  ...
  return {
    ...
    ${Object.values(result.fieldAnalysis)
      ...
      .map(field => {
        ...
        } else if (result.nestedFields.includes(field.path) || result.arrayFields.includes(field.path)) {
